...
while (E_err > (1e-8) or Dm_ > (1e-8)) and not item > 200:
    ...
    Dm_old = Dm
    E_HF = E_occ+E_nuc
    print(f"{item:3d}     {E_HF: 12.8f}      {E_err: 12.8f}       {Dm_: 12.8f}")
    item += 1
...
# 将原子轨道积分变换为分子轨道积分
eri_mo =np.einsum("up,vq,uvkl,kr,ls->pqrs",C_,C_,I,C_,C_,optimize=True) 

# 产生D，T，t矩阵
D_iajb = E[:nocc,None,None,None]+E[None,None,:nocc,None]-E[None,nocc:,None,None]-E[None,None,None,nocc:]
t_iajb = eri_mo[:nocc,nocc:,:nocc,nocc:]/D_iajb
T_iajb = 2*t_iajb-t_iajb.transpose(0,3,2,1)

# 计算MP2相关能
E_corr = (T_iajb*t_iajb*D_iajb).sum()
print(f"MP2相关能:{E_corr:12.8f} Hartree")
print(f"总能量：{E_HF+E_corr:12.8f} Hartree")

y = time.time()
print(f"运行总时间：{y-x:.3f}秒")